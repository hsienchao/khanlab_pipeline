	...
	foreach my $locus (@beds) {
		 ...
		 if ($super_loci) {
				...
				if ($chr eq $super_loci->chr && ($start - $super_loci->end) <= $super_loci_cutoff) {
					if ($tad_file) {
						if (inSameTAD($super_loci->start, $super_loci->end, $start, $end, $tad{$chr})==1){
...
# Check if two regions are in the same TAD
# Return values:
# 1 : Both regions are in the same TAD
# 0 : Not in the same TAD
# -1: Enhancer region is not in TAD

sub inSameTAD {
    my ($g_start, $g_end, $e_start, $e_end, $tad_ref) = @_;
	my $found_enh_inTAD = 0;
	my $found_gene_inTAD = 0;
    foreach my $tad (@{$tad_ref}) {
        my $start = @{$tad}[0];
        my $end = @{$tad}[1];
		my $gene_inTAD = (($g_start < $start && $g_end > $start && (($g_end - $start)/($g_end - $g_start) > 0.5)) || ($g_start < $end && $g_end > $end && (($end - $g_start)/($g_end - $g_start) > 0.5)) || ($g_start > $start && $g_end < $end));
		my $enhancer_inTAD = (($e_start < $start && $e_end > $start) || ($e_start < $end && $e_end > $end) || ($e_start > $start && $e_end < $end));
		if ($enhancer_inTAD) {
			$found_enh_inTAD = 1;
		}
		if ($gene_inTAD) {
